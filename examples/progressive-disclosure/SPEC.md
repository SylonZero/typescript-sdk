...
2. **A separate `tools/search` method, leaving `tools/list` as the lightweight surface**. Rejected: changing the meaning of an existing method is a backward-compatibility hazard, and "list" is a poor name for the lightweight surface once `summary` is added.
